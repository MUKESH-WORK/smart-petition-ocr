...
def parse_tamil_address_and_location(address_str: str) -> Dict[str, str]:
    ...
    if hier.get("ward_no"):
        location["ward_no"] = hier["ward_no"]
    if hier.get("ward_name"):
        location["ward_name"] = hier["ward_name"]
    if hier.get("municipality_ward"):
        location["municipality_ward"] = hier["municipality_ward"]
    if hier.get("local_body_type"):
        location["local_body_type"] = hier["local_body_type"]
    if hier.get("zone"):
...
def is_same_person_or_invalid(sig: Optional[str], app: Optional[str]) -> bool:
    """
    Returns True if the closing signatory name is invalid (e.g. an enclosure line,
    annexure, common Tamil banner text) or if it refers to the same applicant
    (e.g. English transliteration vs Tamil name, matching initials, or identical string).
    """
    if not sig or not isinstance(sig, str):
        return True
    
    # Check invalid enclosure, copy, or closing keywords
    INVALID_SIG_TOKENS = [
        "மனு", "நகல்", "மக்கள்", "கையெழுத்து", "இணைப்பு", "சான்றிதழ்", "சான்று",
        "அட்டை", "புகைப்படம்", "ரசீது", "பகுதி", "விண்ணப்பம்", "petition", "copy",
        "enclosure", "annexure", "தங்கள்", "உண்மையுள்ள", "வணக்கம்", "நன்றி",
        "ஆதார்", "வாக்காளர்", "ரேஷன்", "சாதி", "வருமானம்"
    ]
    sig_lower = sig.lower().strip()
    if len(sig_lower) < 2 or any(t in sig_lower for t in INVALID_SIG_TOKENS):
        return True
    ...
    norm_s = re.sub(r'[\s\.\,\(\)\-\:\'\"]', '', sig_lower)
    norm_a = re.sub(r'[\s\.\,\(\)\-\:\'\"]', '', app.lower())
    if not norm_s or not norm_a:
        return True
    if norm_s == norm_a or norm_s in norm_a or norm_a in norm_s:
        return True

    # Common Tamil-English transliteration pairs
    TAMIL_TO_ENG = {
        "அருண்": "arun", "குமார்": "kumar", "அருண்குமார்": "arunkumar",
        "முருகன்": "murugan", "செல்வி": "selvi", "ராஜா": "raja",
        "சுரேஷ்": "suresh", "ரமேஷ்": "ramesh", "கார்த்திக்": "karthik",
        "மகேஷ்": "mahesh", "பிரகாஷ்": "prakash", "செந்தில்": "senthil",
        "சுப்பிரமணிய": "subramani", "துரைராஜ்": "durairaj", "சந்திரசேகர்": "chandrasekar",
        "மணி": "mani", "பாபு": "babu", "விஜய்": "vijay", "தினேஷ்": "dinesh"
    }
    for tam, eng in TAMIL_TO_ENG.items():
        if tam in app and eng in sig_lower:
            return True
...
def parse_door_street_village(addr_str: str) -> Dict[str, Optional[str]]:
    """
    Parses an address string into door_no, street_name, and village/locality.
    Disambiguates street (e.g. பாவானி மெயின் ரோடு) from locality/village (e.g. பெரியார் நகர்).
    """
    res = {"door_no": None, "street_name": None, "village": None}
    if not addr_str:
        return res

    # 1. Door Number extraction
    door_match = re.search(
        r'(?:^|\b)(?:கதவு\s*எண்\s*[:\.]?\s*|Door\s*No\.?\s*[:\.]?\s*|D\.?\s*No\.?\s*[:\.]?\s*)?(\d{1,5}(?:[/-]\d{1,5})?[A-Za-z]?)(?:\s*,\s*|\s+)',
        addr_str,
        re.IGNORECASE
    )
    if door_match:
        res["door_no"] = door_match.group(1).strip()
    ...
    for s in segments:
        ...
        if re.search(r'\b6\d{2}\s*\d{3}\b', clean_s) and len(clean_s) < 25:
            continue
        filtered_segments.append(clean_s)
    ...
    # Clean cross-contamination if street contains village or vice-versa
    if detected_street and detected_village:
        if detected_village in detected_street and detected_street != detected_village:
            detected_street = detected_street.replace(detected_village, '').strip(' ,.-')
        elif detected_street in detected_village and detected_village != detected_street:
            detected_village = detected_village.replace(detected_street, '').strip(' ,.-')

    res["street_name"] = detected_street
    res["village"] = detected_village
    return res
...
def extract_header_entities(header_zone_text: str, full_ocr_text: str = "") -> Dict[str, str]:
    ...
    if idx_sig != -1:
        ...
        for sl in [l.strip() for l in sig_block.split("\n")[1:10] if l.strip()]:
            # Skip enclosure lines (e.g. 1. பகுதி மக்கள் கையெழுத்து மனு - 1 நகல், 2. புகைப்படம், 3. ஆதார் நகல்)
            if any(encl in sl.lower() for encl in ["இணைப்பு", "நகல்", "மனு", "புகைப்படம்", "சான்றிதழ்", "சான்று", "அட்டை", "மக்கள்", "கையெழுத்து", "enclosure", "copy", "annexure"]):
                continue
    ...
    if entities.get("address"):
        parsed_loc = parse_door_street_village(entities["address"])
        if parsed_loc.get("door_no"):
            entities["door_no"] = parsed_loc["door_no"]
        if parsed_loc.get("street_name"):
            entities["street_name"] = parsed_loc["street_name"]
        if parsed_loc.get("village"):
            entities["village"] = parsed_loc["village"]
    ...
    # Match administrative hierarchy (Division, Taluk, Firka, District) dynamically from JSON
    hier = location_matcher.match_hierarchy(
        address_text=addr_str,
        village=entities.get("village"),
        street=entities.get("street_name"),
        detected_taluk=entities.get("taluk")
    )
    if hier.get("district"):
        entities["district"] = hier["district"]
    if hier.get("revenue_division"):
        entities["revenue_division"] = hier["revenue_division"]
    if hier.get("taluk"):
        entities["taluk"] = hier["taluk"]
    if hier.get("firka"):
        entities["firka"] = hier["firka"]
    if hier.get("ward"):
        entities["ward"] = hier["ward"]
    if hier.get("municipality_ward"):
        entities["municipality_ward"] = hier["municipality_ward"]
    if hier.get("local_body_type"):
        entities["local_body_type"] = hier["local_body_type"]
...
class EntityExtractor:
    ...
    def _extract_structural_entities(self, full_text: str, page_number: int) -> List[Dict[str, Any]]:
        ...
        cand_sig = header_parsed.get("complainant_signatory")
        cand_app = header_parsed.get("petitioner_name", "")
        if cand_sig and not is_same_person_or_invalid(cand_sig, cand_app) and not any(e["entity_type"] == "complainant_signatory" for e in entities):
            entities.append({
                "entity_type": "complainant_signatory",
                "entity_value": cand_sig,
                "confidence": 0.98,
                "source_page": page_number,
