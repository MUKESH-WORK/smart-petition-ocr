...
def test_taxonomy_validator_alignment():
    """Validates that taxonomy validator aligns classifications against the official CM Helpline taxonomy."""
    from services.taxonomy_matcher import taxonomy_matcher
    res = taxonomy_matcher.match(detected_type="Pension", detected_subtype="Destitute Widow Pension Scheme (DWPS)")
    assert res["department"] == "Revenue and Disaster Management (REV)"
    assert "Pension" in res["grievance_type"]
    assert "Destitute Widow Pension" in res["grievance_subtype"]
...
def test_enterprise_health_endpoint():
    ...
    with TestClient(app) as client:
        ...
        assert "OCR" in data["components"]["ocr"]["engine"]
        assert data["components"]["ocr"]["dpi"] >= 150
